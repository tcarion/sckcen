...
function calc_avg_spread_accu(curdf)
    ...
    outdf = combine(groupby(accu, [:n_forecasts]), :H10_std => mean => :H10_std_avg)
    print(outdf)
...
function calc_avg_spread(df)
    outdf = @chain df begin
        ...
        combine(groupby(_, [:fcstart]), :H10_std => mean => :H10_std_avg)
    end
...
meteo_avg_ens = combine(groupby(meteo_ens_stats, [:fc_label, :leadtime1]), :speed_std => mean => :speed_std_avg)
...
ax = Axis(f_spread_evol[1,1],
    ...
    xticks = (1:npoints, ["$lt hours" for lt in meteo_avg_ens.leadtime1]),
)

scatterlines!(ax, reverse(1:npoints), reverse(fp_df_avg.H10_std_avg |> ustrip), label="flexpart", color=COLORS[1])
scatterlines!(ax, reverse(1:npoints), fp_df_avg_acc.H10_std_avg |> ustrip, label="flexpart", color=COLORS[1], linestyle=:dash)

scatterlines!(ax, reverse(1:npoints), reverse(gs_df_avg.H10_std_avg |> ustrip), label="gaussian", color=COLORS[2])
scatterlines!(ax, reverse(1:npoints), gs_df_avg_acc.H10_std_avg |> ustrip, label="gaussian", color=COLORS[2], linestyle=:dash)

scatterlines!(ax, reverse(1:npoints), reverse(al_df_avg.H10_std_avg |> ustrip), label="all", color=COLORS[3])
scatterlines!(ax, reverse(1:npoints), al_df_avg_acc.H10_std_avg |> ustrip, label="all", color=COLORS[3], linestyle=:dash)

scatterlines!(ax, reverse(1:npoints), reverse(meteo_avg_ens.speed_std_avg), label="wind speed", color=COLORS[4])
