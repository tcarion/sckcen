...
```{julia}
# | label: fig-spread
# | fig-cap: "Temporal and spatial average of ensemble spread across all valid detection stations, showing the impact of progressively incorporating earlier forecast initializations (from 0 to 48 hours prior to the first initialization at 2019-05-15 00:00)."

# Plot the evolution of the spread with the initialization time
function stats_by_fcstart(curdf)
    fcstarts = [
    ...
    vcat(fcstart_stats...; cols=:union)
end

fpstats = @rsubset stats_by_fcstart(@rsubset df :simtype == "flexpart" && :isensemble == true) :receptorName in filtered_sensors
gsstats = @rsubset stats_by_fcstart(@rsubset df :simtype == "gaussian" && :isensemble == true) :receptorName in filtered_sensors
allstats = @rsubset stats_by_fcstart(@rsubset df :isensemble == true) :receptorName in filtered_sensors

fpstats_avg = combine(groupby(fpstats, [:n_forecasts]), :H10_std => mean => :spread_avg)
gsstats_avg = combine(groupby(gsstats, [:n_forecasts]), :H10_std => mean => :spread_avg)
allstats_avg = combine(groupby(allstats, [:n_forecasts]), :H10_std => mean => :spread_avg)

f_spread_evol = Figure(; 
)
ax_spread_evol = Axis(f_spread_evol[1, 1],
    # title = "Evolution of the spread average when adding forecast initializations.",
    # ylabel = L"\sigma",
    ylabel = "Average spread",
    xlabel = "Number of initializations",
    # xticks = (1:length(mean_and_median_spread.forecast_start), Dates.format.(mean_and_median_spread.forecast_start, "ddth of \\Ma\\y at HHh"))
)

scatterlines!(ax_spread_evol, fpstats_avg[!, :n_forecasts], fpstats_avg[!, :spread_avg] .|> ustrip, label="flexpart")
scatterlines!(ax_spread_evol, gsstats_avg[!, :n_forecasts], gsstats_avg[!, :spread_avg] .|> ustrip, label="gaussian")
scatterlines!(ax_spread_evol, allstats_avg[!, :n_forecasts], allstats_avg[!, :spread_avg] .|> ustrip, label="both")
ylims!(ax_spread_evol, (0, 0.65))
axislegend(ax_spread_evol, position=:rb)

save("figures/fig-spread.png", f_spread_evol)

f_spread_evol
```

To assess how forecast initialization times affect ensemble uncertainty, we computed the time- and station-averaged ensemble spread (@fig-spread). The results demonstrate a systematic increase in spread with each additional forecast initialization, culminating in a 42% enhancement for the full ensemble compared to using only the most recent forecast. Interestingly, individual model ensembles (FLEXPART or Gaussian plume alone) sometimes exhibit greater spread than the combined ensemble. This occurs because FLEXPART frequently produces tighter concentration distributions (lower spread) while maintaining similar ensemble means to the Gaussian plume model, thereby reducing the overall variability in the full ensemble.

```{julia}
#| echo: false
spread_increase = (allstats_avg.spread_avg[end] / allstats_avg.spread_avg[1] - 1) * 100
sspread_increase = @sprintf "%d" spread_increase
# md"For the full ensemble, the spread is a increased by $(sspread_increase)%"
sspread_increase == "42" || error("The percentage must be updated")
```
...
# Conclusion
This study examined the influence of multi-model and multi-forecast initializations on ensemble spread in short-range dispersion modelling. Results indicate that increasing the number of forecast initializations generally enhances ensemble spread, whereas the multi-model approach can reduce it—particularly when the participating models share similar ensemble means.

In all cases, the ensembles analysed appeared under-dispersive. Further investigation could assess whether adding more forecast initialization members would lead to a greater spread. Ensemble skill was addressed only briefly and qualitatively in this work, as the limited dataset (24 cases) renders more robust quantitative assessments (e.g., rank histograms, bias, root-mean-square error) statistically unreliable. A more comprehensive evaluation of how multi-modeling and additional forecast initializations affect ensemble skill would require a substantially larger set of observations.
